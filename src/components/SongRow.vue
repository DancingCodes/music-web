<template>
  <div class="flex items-center gap-5 px-3 py-3 rounded-xl cursor-pointer group transition-colors duration-200"
    :class="isPlaying ? 'bg-red-50 dark:bg-red-900/20' : 'hover:bg-gray-100 dark:hover:bg-gray-800'" @click="$emit('play')">
    <span class="w-6 text-xs text-right tabular-nums shrink-0" :class="isPlaying ? 'text-red' : 'text-gray-400 dark:text-gray-500'">{{
      index }}</span>
    <img
    ...
    <div class="flex-1 min-w-0">
      ...
      <div class="text-xs text-gray-500 dark:text-gray-400 truncate mt-0.5">{{ music.artists }} · {{ formatTime(music.duration_ms) }}</div>
    </div>
    <div @click.stop="$emit('delete')"
      class="cursor-pointer text-gray-400 dark:text-gray-500 hover:text-red opacity-0 group-hover:opacity-100 shrink-0 transition-colors duration-200">
