<template>
  <div class="flex items-center gap-4 px-3 py-2.5 rounded-lg cursor-pointer group"
    :class="isPlaying ? 'bg-red/5' : 'hover:bg-neutral-900'" @click="$emit('play')">
    <span class="w-6 text-xs text-right tabular-nums shrink-0" :class="isPlaying ? 'text-red' : 'text-neutral-600'">{{
      index }}</span>
    <img
