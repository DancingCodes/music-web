<template>
  <div class="flex items-center gap-4 px-3 py-2.5 rounded-lg cursor-pointer group"
    :class="isPlaying ? 'bg-red-50' : 'hover:bg-gray-500'" @click="$emit('play')">
    <span class="w-6 text-xs text-right tabular-nums shrink-0" :class="isPlaying ? 'text-red' : 'text-gray-500'">{{
      index }}</span>
    <img
    ...
    <div @click.stop="$emit('delete')"
      class="cursor-pointer text-gray-500 hover:text-red-500 opacity-0 group-hover:opacity-500 shrink-0">
