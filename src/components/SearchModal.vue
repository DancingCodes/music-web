<template>
  <div v-if="show" class="fixed inset-0 z-50 flex items-start justify-center pt-20" @click.self="close">
    ...
    <div
      class="relative bg-white rounded-lg w-full max-w-2xl max-h-[70vh] flex flex-col mx-4 border border-gray-500">
      <div @click="close" class="absolute top-3 right-3 text-gray-500 hover:text-gray-500 cursor-pointer z-10">
        <X class="w-5 h-5" />
      </div>
      <div class="flex items-center gap-3 px-4 h-12 border-b border-gray-500">
        <Search class="w-4 h-4 text-gray-500 shrink-0" />
        <input v-model="keyword" @keyup.enter="onSearch" placeholder="搜索网易云歌曲..."
          class="flex-1 h-10 px-4 box-border bg-transparent border border-solid border-gray-500 rounded-lg text-gray-500 text-sm outline-none placeholder:text-gray-500" />
      </div>
      ...
      <div class="flex-1 overflow-y-auto px-4 pt-3" style="scrollbar-width:none;-ms-overflow-style:none">
        ...
        <div v-for="m in results" :key="m.id" class="flex items-center gap-3 py-3 border-b border-gray-500">
          ...
          <div v-if="!savedIds.has(m.id)" @click="onSave(m)"
            class="text-xs px-3 py-1 rounded-full bg-red-500 hover:bg-red-500 text-white cursor-pointer"
            :class="{ 'opacity-50 pointer-events-none': savingIds.has(m.id) }">
            <Loader2 v-if="savingIds.has(m.id)" class="w-3.5 h-3.5 animate-spin" />
            <span v-else>保存</span>
          </div>
          <span v-else class="text-xs text-gray-500">已保存</span>
        </div>

        <div v-if="results.length" class="text-center py-4">
          <div v-if="results.length < total" @click="loadMore"
            class="text-sm cursor-pointer text-gray-500 hover:text-red-500">{{ loading ? '加载中...' : '加载更多' }}</div>
