<template>
  <Teleport to="body">
    <div v-if="show" class="fixed inset-0 z-50 flex items-start justify-center pt-20" @click.self="close">
      ...
      <div class="relative bg-neutral-900 rounded-lg w-full max-w-lg max-h-[70vh] flex flex-col mx-4 border border-neutral-800">
        ...
        <div class="flex-1 overflow-y-auto px-4" style="scrollbar-width:none;-ms-overflow-style:none">
          <div v-if="keyword && !loading && results.length === 0" class="text-center py-16 text-neutral-500 text-sm">没有找到相关歌曲</div>

          <div v-if="!keyword && results.length === 0" class="flex flex-col items-center justify-center py-16 text-neutral-500">
          ...
          <div
            ...
            class="flex items-center gap-3 py-3 border-b border-neutral-800"
          >
            <div class="flex-1 min-w-0">
              <div class="text-sm truncate">{{ m.name }}</div>
              <div class="text-xs text-neutral-500 truncate mt-0.5">{{ artistNames(m) }} · {{ formatTime(m.duration) }}</div>
            </div>
            <div
          ...
          <div v-if="results.length > 0" class="text-center py-4">
            <div v-if="results.length < total" @click="loadMore" class="text-sm cursor-pointer text-neutral-400 hover:text-red-500">{{ loading ? '加载中...' : '加载更多' }}</div>
